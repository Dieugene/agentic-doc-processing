...
| Экстракция текста | VLM-OCR для всех форматов (через конвертацию → PDF → PNG) |
| Обработка таблиц | Гибридная: VLM-классификация → числовые из исходника (Pandas), текстовые через VLM-OCR |
| Модели агентов | Дешёвые модели (Claude Haiku, GPT-4o-mini, локальные) с SGR-структурированием |

> **Подробнее:** См. ADR-001 (форматы и обработка таблиц), ADR-002 (мультидокументность), ADR-003 (интеграция VLM-OCR)
...
**Метод:** Извлечение из исходного файла → Pandas

- VLM-OCR классифицирует таблицу как NUMERIC
- Table Extractor загружает таблицу из исходного Excel/PDF
- Таблица сохраняется в DataFrame для вычислений
- Агент генерирует код для извлечения/вычислений через Pandas
- Исключаются галлюцинации LLM на арифметике

### 5.3. Обработка таблиц типа Б (текстовые матрицы)

**Метод:** Cell Flattening через VLM-OCR

- VLM-OCR классифицирует таблицу как TEXT_MATRIX
- VLM-OCR выполняет Cell Flattening на основе изображения таблицы
- Преобразование в список утверждений: `[Заголовок строки] + [Заголовок столбца] → [Содержимое ячейки]`
...
Результат Cell Flattening (через VLM-OCR):
...
**Обоснование:** Снижение риска промпт-инъекций (текст таблицы идет через изображение)
...
Workspace {
  ...
  collections: Map<collection_id, DocumentCollection>
...
DocumentCollection {
  ...
  primary_document_id: string   // основной документ
}
...
> **Подробнее:** См. ADR-002 (каждый файл = отдельный Document, группировка в DocumentCollection)
...
```
Вход: PDF / Excel / DOCX / ZIP файл

1. Если ZIP → распаковка → рекурсивная обработка каждого файла
2. Конвертация в PDF (для DOCX/Excel)
3. Рендер страниц в PNG
4. VLM-OCR extraction (batch prompts):
   - Текст документа
   - Структура (заголовки, иерархия)
   - Классификация таблиц (NUMERIC / TEXT_MATRIX)
5. Обработка таблиц:
   - NUMERIC → извлечь из исходного файла → Pandas
   - TEXT_MATRIX → Cell Flattening через VLM-OCR
6. Построение DocumentSkeleton

Выход: DocumentSkeleton
```

> **Подробнее:** См. ADR-001 (унифицированный VLM-OCR pipeline, гибридная обработка таблиц), ADR-003 (VLM-OCR API и оптимизация batch prompts)
